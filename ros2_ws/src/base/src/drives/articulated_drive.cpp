...
articulatedWheelSpeed ArticulatedDrive::inverseKinematics(geometry_msgs::msg::Twist cmdVelMsg, double angle)
{
    ...
    if (targetSpeed_ >= 0)
    {
        ...
        retVal.Rear.leftWheel = (cos(angle)/wheelRadius_ + (axesLength_ * sin(angle))/(frontLength_ * wheelRadius_)) * targetSpeed_ + ((-1* frontLength_*sin(angle) / (2*wheelRadius_))- (axesLength_ * cos(angle))/(2*wheelRadius_)) * targetOmega_;
        retVal.Rear.rightWheel= (cos(angle)/wheelRadius_ - (axesLength_ * sin(angle))/(frontLength_ * wheelRadius_)) * targetSpeed_ + ((-1* frontLength_*sin(angle) / (2*wheelRadius_))+ (axesLength_ * cos(angle))/(2*wheelRadius_)) * targetOmega_;
    }
